...
//				$del_path = $del_path.$file;
			...
			if ($do_redir) {
				$redir_url = 'remote_files.php?files_path='.$files_path.'&files_url='.$files_url.'&file_type='.$file_type.'&del_path='.$del_path.'&lang='.$lang;
	...
	if (isset($local_del_path)) {
		$redir_url = 'remote_files.php?files_path='.$files_path.'&files_url='.$files_url.'&file_type='.$file_type.'&del_path='.$local_del_path.'&lang='.$lang.'&action=reload';
		if (isset($_GET['re_session_id'])) $redir_url .= '&re_session_id='.$re_session_id;
		Header('Location: '.$redir_url);
		exit;
...
<table border="0" cellspacing="0" cellpadding="2" width="100%" height="100%" class="re_dialog"><tr vAlign="top"><td class="re_dialog">
